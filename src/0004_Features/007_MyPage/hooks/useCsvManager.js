    ...
    reader.onload = async (event) => {
      try {
        ...
        for (let i = 0; i < validCount; i++) {
          ...
          // 2. 주소 및 좌표 결정 로직
          //    Case A: CSV에 좌표가 있다 -> 주소 없으면 좌표로 주소 검색 (역지오코딩)
          //    Case B: CSV에 좌표가 없다 -> 주소로 좌표 검색 (지오코딩)
          ...
          if (csvLat && csvLng) {
             ...
             // ★ [추가됨] 주소가 비어있으면 좌표를 이용해 주소를 채워넣음
             if (!address) {
                try {
          ...
          chunkBuffer.push({
            ...
            address: address || '', // 여기서 찾아낸 주소가 들어갑니다.
